...
parser.add_argument('--subset', default=None, type=float, choices=[0.75, 0.5, 0.1, 0.25])
best_acc1 = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.subset:
        step = int(1. / args.subset)
        idxs = list(range(1, len(train_dataset), step))
        # print('idxs:', idxs)
        if args.subset > 0.5:
            step = int(1. / (1. - args.subset))
            idxs0 = list(range(1, len(train_dataset), step))
            idxs = list(set(range(len(train_dataset))) - set(idxs0))
