...
if __name__=='__main__':
    ...
    TEACHER_PATH = '/import/home/xliude/PycharmProjects/nex_project/multi_classes/DeiT/results/08-12_22-25_Resnet50_imbCIFAR10_01/checkpoint.pth'
    DIST_TYPE = 'hard'
    DIST_ALPHA = 0.5
    DIST_TAU = 1.0
    ...
    if TEACHER_PATH.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            TEACHER_PATH, map_location='cpu', check_hash=True)
    else:
        checkpoint = torch.load(TEACHER_PATH, map_location='cpu')
    teacher_model.load_state_dict(checkpoint['model'])
    teacher_model.to(device)
    teacher_model.eval()
    criterion = DistillationLoss(criterion, teacher_model, DIST_TYPE, DIST_ALPHA, DIST_TAU)


    time_str = datetime.strftime(datetime.now(), '%m-%d_%H-%M')
    time_str = time_str + "_Deit_kd" + "_imbCIFAR10_01_imb_sampler"
