...
class SMALLCIFAR10(torchvision.datasets.CIFAR10):
    cls_num = 10
    def __init__(self, root, number=50, rand_number = 0, train=True, transform=None, target_transform=None, download=False):
        super(SMALLCIFAR10,self).__init__(root, train, transform, target_transform, download)
        np.random.seed(rand_number)
        img_num_list = [number] * self.cls_num
        self.get_data(img_num_list)
    
    def get_data(self, img_num_per_cls):
        new_data = []
        new_targets = []
